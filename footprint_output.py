...
# file_precompute_peaks_or_hashes(analyzer, filename, './data')
...
Usage: footprint_output [options] [<file>]...

Options:
  -d <dbase>, --dbase <dbase>     Fingerprint database file
  -M <hashes_not_peaks>, --hashes-not-peaks <hashes_not_peaks>     Output hashes, not peaks
...
def main(argv):
    ...
    if args["--hashes-not-peaks"]:
        args["--hashes-not-peaks"] = bool(int(args["--hashes-not-peaks"]))
    else:
        args["--hashes-not-peaks"] = True
    ...
    r = file_precompute_peaks_or_hashes(analyzer, file.strip('\r\n'), './data', None, args["--hashes-not-peaks"])
    #print(r)

    numpy.savetxt(sys.stdout, r, fmt='%.10f')

    #import code; code.interact(local=dict(globals(), **locals()))
    #numpy.savetxt('test.txt', peaks)
